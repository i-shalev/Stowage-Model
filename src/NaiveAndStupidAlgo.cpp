...
void NaiveAndStupidAlgo::getInstructionForCargo(const std::string& outputPath) {
    ...
    //and now the stupid part:sd
    bool found=false;
    for(int level = ship->getPlan().getNumFloors()-1; level>=0 && !found; level++){
        for(int i=0; i<ship->getPlan().getLength() && !found; i++){
            for(int j=0; j<ship->getPlan().getWidth() && !found; j++){
                if(ship->getPlan().getFloor(level)->getContainerAtPosition(i,j) != nullptr &&
...
int NaiveAndStupidAlgo::emptyPlacesInPosition(int i, int j, const string& portSymbol){
    int sum = 0;
    for(int level=0; level<ship->getPlan().getNumFloors(); level++){
        if(ship->getPlan().getFloor(level)->getContainerAtPosition(i,j)== nullptr ||
           (!ship->getPlan().getFloor(level)->getContainerAtPosition(i,j)->getBlocked() &&
...
bool NaiveAndStupidAlgo::checkContainer(Container* cont) {
