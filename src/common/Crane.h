...
//using namespace std;
enum result{ SUCCESS=0, DEST_NOT_IN_LIST=1, INVALID_INDEX=2, FULL_PLACE=3, EMPTY_BELOW=4, CONTAINER_ABOVE=5, WRONG_CONTAINER=6, NOTHING_HERE=7};
class Crane{
    Ship* curShip;
    Port* curPort;
...
public:
    ...
    result Move(const std::string &contId, int level, int i, int j, int toLevel, int toI, int toJ);
};
